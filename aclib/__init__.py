...
def get_url(db_name=None, schema_name=None):
    username_filename = os.getenv("ACLIB_USERNAME_FILENAME", os.path.expanduser("~") + "/secrets/username")
    snowflake_access_token_filename = os.getenv("ACLIB_SNOWFLAKE_ACCESSS_TOKEN_FILENAME", os.path.expanduser("~") + "/secrets/snowflake_access_token")
    ...
    if not os.path.exists(username_filename) and not os.path.exists(snowflake_access_token_filename):
        ...
        cred.read([username_filename, snowflake_access_token_filename])
        credd = dict(cred.items(credential_section))
        snowflake_host = credd.get('host', "alphacruncher.eu-central-1")
        url = 'snowflake://' + credd['username'] + ':' + credd['snowflake_access_token'] + '@' + snowflake_host + '/?warehouse=' + credd['uid']
        masked_url = 'snowflake://' + credd['username'] + ':********' + '@' + snowflake_host + '/?warehouse=' + credd['username']
